...
public class App {
  ...
  private static void runApp(String[] args) throws InterruptedException, IOException, ExecutionException {
    ...
    if (
        ((args.length == 3) && (!(requestType = RequestType.getType(args[0])).equals(RequestType.decompress))) ||
            ((args.length == 4) && (!(requestType = RequestType.getType(args[0])).equals(RequestType.compress))) ||
